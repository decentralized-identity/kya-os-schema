...
const FAQ = [
  ...
  },
  {
    ...
    a: "The full protocol spec lives at [kya-os.org](https://kya-os.org), which spans every binding — MCP, HTTP, and more. For MCP specifically, see [kya-os.org/mcp](https://kya-os.org/mcp); its reference implementation and these schemas are in the donated core repo, [decentralized-identity/kya-os-mcp](https://github.com/decentralized-identity/kya-os-mcp).",
  },
];
...
// Favicon: the mark in white on a dark rounded brand tile. SVG only — every
// current browser honours an SVG favicon, so no raster step is needed. The mark
// (197×227) is scaled to ~148px and centred inside a 256×256 squircle tile.
const FAVICON_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256"><rect width="256" height="256" rx="52" fill="#0a0a0a"/><g transform="translate(64,54) scale(0.65)" fill="#ffffff"><path d="${LOGO_PATH}"/></g></svg>\n`;
...
function renderHeaders() {
  return [
    ...
    "/favicon.svg", "  Content-Type: image/svg+xml", "  Cache-Control: public, max-age=604800",
    "",
  ].join("\n");
}
